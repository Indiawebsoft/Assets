...
class AddLinkAsset extends StatefulWidget {
  AddAssetModel selectedItem;
  // List<Categories> allCategory;
String eidtitem;
  AddLinkAsset(this.selectedItem, this.eidtitem,{super.key});

  @override
  State<AddLinkAsset> createState() => _AddLinkAssetState();
}

class _AddLinkAssetState extends State<AddLinkAsset> {
  ...
  void initState() {
    ...
    if(widget.eidtitem!=""){
      addAssetData=widget.selectedItem;
      addAssetData.item_image =addAssetData.item_image;
              // addAssetData.item = widget.selectedItem.id;
              // addAssetData.itemName = = widget.selectedItem.itemName;
              // addAssetData.itemForm = iit.formdata;
              // addAssetData.subCategory = iit.subcategory_id;
              // addAssetData.subCategoryName =  iit.subcategotyname;
    }else{
      addAssetData.category = widget.selectedItem.category;
      addAssetData.categoryName = widget.selectedItem.categoryName;
      addAssetData.item_image =widget.selectedItem.item_image;
      // addAssetData.subCategory = widget.selectedItem.subCategory;
      // addAssetData.subCategoryName = widget.selectedItem.subCategoryName;

      addAssetData.add_button = "No";
      addAssetData.parent = widget.selectedItem.id;
    }
  ...
  Widget build(BuildContext context) {
    ...
    addAssetData.subCategoryName = widget.selectedItem.subCategoryName;
    if(widget.eidtitem!="") {
      addAssetData.parent = widget.selectedItem.parent;
    }else{
      addAssetData.parent = widget.selectedItem.id;
    }
    return Container(
  ...
  Widget buildProfileUI() {
    ...
    return SingleChildScrollView(
        child:  FutureBuilder(
        future: appController.getAllAssetItems(
        login_user!.id),
  ...
  _getPopupFormFields(StateSetter popup1setState) {
    ...
    return Column(children: [
      ...
      i++) ...[
        if (addLinkAssetData.itemForm!.form_name!.files![i].type ==
        ...
        ] else if (addLinkAssetData.itemForm!.form_name!.files![i].type == "p") ...[
          _getLabel(addLinkAssetData.itemForm!.form_name!.files![i].heading!,
              addLinkAssetData.itemForm!.form_name!.files![i].isrequerd!),
          TextField(
            minLines: 3,
          ...
          DropdownSearch<String>(
            ...
            selectedItem: addLinkAssetData.itemForm!.form_name!.files![i].value,
          ),
        ] else if (addLinkAssetData.itemForm!.form_name!.files![i].type ==
            "multipal") ...[
          _getLabel(addLinkAssetData.itemForm!.form_name!.files![i].heading!,
          ...
          _getLabel(addLinkAssetData.itemForm!.form_name!.files![i].heading!,
              addLinkAssetData.itemForm!.form_name!.files![i].isrequerd!),
          for (var item in addLinkAssetData.itemForm!.form_name!.files![i].option!)
            CheckboxListTile(
              title: Text(item.toUpperCase()),
          ...
          Padding(
              ...
              child: Container(
                  child: Align(
                    alignment: Alignment.centerRight,
                    child: OutlinedButton(
                      onPressed: (){ _showPopup2(popup1setState); },
                      style: OutlinedButton.styleFrom(
                        //<-- SEE HERE
                        side: BorderSide(width: 1.0),
                      ),
  ...
  _getPopupFormFields2(StateSetter popup1setState) {
    ...
    for (AddAssetModel tempItems in oldListAsset2!) {
      oldtempitemDropDownList2.add("${tempItems.itemName!}(code: ${tempItems.id!})");
    }
    ...
    return Column(children: [
      ...
      (oldListAsset2 != null && oldListAsset2!.isNotEmpty) ?  _getLabel(
          "OR Add New","NO"):Container(),
      ...
      Padding(
          ...
          child: ElevatedButton(
            ...
            onPressed: () {_onSubmitPopu2Click(popup1setState);},
            child: const Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(
        ...
        return StatefulBuilder(
            builder: (context, setState) {
              return  AlertDialog(
                ...
                content: Container(
                  width: MediaQuery.of(context).size.width-150,//double.maxFinite,
                  height: MediaQuery.of(context).size.height-300, // Adjust height according to your needs
                  child: SingleChildScrollView(
                    child: FutureBuilder(
                        future: appController.getAllMyFreeAssetItems(
                            login_user!.id),
                        builder: (context, AsyncSnapshot<GetAssetItems> snapshot) {
                          print(snapshot);
                          // categories = GetStorage().read(Constant.CATEGORIES);
  ...
  _onSubmitPopu2Click(StateSetter popup1setState) async {
    ...
    addLinkAssetData2.headingName = addLinkAssetData2.itemForm!.form_name!.files![0].value;
  ...
  _onSubmitClick() async {
    ...
    if(addAssetData.itemForm!.form_name!.files![0].type=="file"){
      addAssetData.headingName =
          addAssetData.itemForm!.form_name!.files![1].value;
    }else {
      addAssetData.headingName =
          addAssetData.itemForm!.form_name!.files![0].value;
    }

    addAssetData.item_image =assetimagefile;
